...
UVCPreview::UVCPreview(uvc_device_handle_t *devh)
    ...
    mPixelFormat = PIXEL_FORMAT_NV21;
    EXIT();
}
...
int UVCPreview::yuyv_to_yuv420p(const unsigned char *in, unsigned char *out, unsigned int width,
                                unsigned int height) {
    unsigned char *y = out;
...
void UVCPreview::do_capture_callback(JNIEnv *env, uvc_frame_t *frame) {
    ...
    if (LIKELY(frame)) {
        ...
        yuyv_to_yuv420p((const unsigned char *) callback_frame->data, yuv420_buf, 640, 480);

        AvcEncoder::getInstance().feedData(yuv420_buf);

        __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG, "%s %d", "Native构建相机预览",
                            callback_frame->frame_format);
